...
Backend URL + secret resolve in priority order:
  1. configure(backend_url, secret)   - the Maestro Case passes the public URL as the
                                         `backend_url` agent input - main.py calls configure(...)
  2. env  FOREMAN_BACKEND_URL / FOREMAN_INGEST_SECRET            (local .env)
  3. UiPath Assets  FOREMAN_BACKEND_URL (Text) / FOREMAN_INGEST_SECRET (Credential)  (cloud)
...
def _asset_text(name: str) -> str | None:
    """Read a Text asset's value from the agent's execution-folder context."""
    try:
        from uipath.platform import UiPath  # imported lazily so local runs need no UiPath

        a = UiPath().assets.retrieve(name)
        return getattr(a, "value", None) or getattr(a, "string_value", None)
    except Exception:  # noqa: BLE001 - no UiPath context locally; fall through to defaults
        return None


def _asset_credential(name: str) -> str | None:
    """Read a Credential asset's password (where FOREMAN_INGEST_SECRET is stored)."""
    try:
        from uipath.platform import UiPath  # imported lazily

        return UiPath().assets.retrieve_credential(name)
...
def _resolve() -> tuple[str, str]:
    ...
    if _resolved is not None:
        return _resolved
    # The SAME two assets the other FOREMAN agents use: FOREMAN_BACKEND_URL (Text)
    # and FOREMAN_INGEST_SECRET (Credential -> password). Default folder context.
